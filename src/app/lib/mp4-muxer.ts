...
export class MP4Muxer {
  ...
  finalize(): Uint8Array {
    ...
    const stblPre = box('stbl', stsd, table.stts, table.stsc, table.stsz, table.stss);
    ...
    const mdiaPre = box('mdia', mdhd, hdlr, box('minf', vmhd, dinf, stblPre));
    const trakPre = box('trak', tkhd, mdiaPre);
    const mvhd = fullBox(
      'mvhd',
      0,
      0,
    ...
    const tempMoov = box('moov', mvhd, trakPre);
    const mdatDataOffset = ftyp.length + tempMoov.length + 8;
