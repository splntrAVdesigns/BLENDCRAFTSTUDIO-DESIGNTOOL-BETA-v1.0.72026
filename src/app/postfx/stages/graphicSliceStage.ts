...
export function createGraphicSliceStage(): PostProcessStage {
  ...
  return {
    id: 'slice',
    material,
    // Two independent audio-modulatable params (amount AND rate) — this is
    // the stage StageOverrides was generalized from a single number for.
    // Must be manually enabled first, audio only modulates.
