...
export const SHARED_ANIMATION_HELPERS = `
  ...
  vec2 applyWaveField(vec2 uv, vec2 center, float baseAngleDeg) {
    ...
    float t = uAnimTime;
    float travel = dot(p, dir) * 10.0 + t * 1.75;
    float cross = dot(p, perp) * 6.5;

    float primary = sin(travel + sin(cross * 0.75 + t * 0.35) * 0.65);
    float secondary = sin(travel * 0.58 - t * 0.62 + sin(cross * 1.15 - t * 0.21) * 1.15);
  ...
  vec2 applyMorphField(vec2 uv, vec2 center) {
    ...
    float t = uAnimTime;
    vec2 uvA = p + vec2(
      sin(p.y * 4.0 + t * 1.2) * 0.03,
      cos(p.x * 3.0 + t * 1.0) * 0.03
    ) * uAnimIntensity;
    vec2 uvB = p + vec2(
      cos(p.y * 7.0 + 1.2 + t * 0.8) * 0.04,
      sin(p.x * 5.0 - 0.8 + t * 1.4) * 0.04
    ) * uAnimIntensity;
    float morphMix = 0.5 + 0.5 * sin(t * 0.23 + sin(t * 0.07) * 0.8);
  ...
  vec2 applyRippleField(vec2 uv, vec2 center) {
    ...
    vec2 c1 = vec2(sin(t * 0.27) * 0.26, cos(t * 0.33) * 0.20);
    vec2 c2 = vec2(cos(t * 0.19 + 1.3) * 0.34, sin(t * 0.29 - 0.7) * 0.28);
    vec2 c3 = vec2(sin(t * 0.41 - 0.9) * 0.22, sin(t * 0.23 + 0.6) * 0.34);
    ...
    float w1 = sin(r1 * 24.0 - t * 2.2 + sin(t * 0.31) * 0.8);
    float w2 = sin(r2 * 20.0 - t * 1.7 + 1.3);
    float w3 = sin(r3 * 28.0 - t * 2.6 - 0.9);

    vec2 ripple = n1 * w1 * exp(-r1 * 1.8) * 0.030;
    ripple += n2 * w2 * exp(-r2 * 1.6) * 0.028;
    ripple += n3 * w3 * exp(-r3 * 1.9) * 0.024;
    ripple += vec2(sin((uv.y + uv.x) * 9.0 - t * 1.4), cos((uv.x - uv.y) * 8.0 + t * 1.1)) * 0.010;
  ...
  vec2 applyKaleidoField(vec2 uv, vec2 center, float segments) {
    ...
    float t = uAnimTime;
    float facet = sin(a * 12.0 + r * 10.0 + t * 1.8 + sin(t * 0.21) * 0.8);
    float facetAmp = (0.014 + 0.028 * animSoftMod()) * uAnimIntensity;
    facetAmp *= (0.7 + edgeWeight * 0.8);
    float tunnel = sin(r * 20.0 + t * 2.4) * 0.02 * uAnimIntensity;
    float foldJitter = sin(t * 1.35 + r * 7.5) * 0.028 * uAnimIntensity;
  ...
  vec2 applyTurbulenceField(vec2 uv, vec2 center) {
    ...
    vec2 flow3 = vec2(
      sin(dot(p, vec2(8.0, 5.0)) - t * 0.73),
      cos(dot(p, vec2(-6.0, 9.0)) + t * 0.58)
